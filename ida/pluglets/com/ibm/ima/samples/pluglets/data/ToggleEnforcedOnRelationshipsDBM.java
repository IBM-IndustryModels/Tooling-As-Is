...
/**
 * The purpose of this pluglet is to turn *OFF* the flag on Enforced to Not Enforced for FK's in a physical data model
 */

public class ToggleEnforcedOnRelationshipsDBM extends Pluglet implements DataPluglet {
	...
	public String getName() {
		return "Physical:Set FK Relationships To Not Enforced";
	}
	...
	public void plugletmain(String[] args) throws FileNotFoundException {
		...
		editingDomain.getCommandStack().execute(new RecordingCommand(editingDomain) {
				...
				while (selectionListIter.hasNext()) {
					...
					if (nxtObj != null) {
						if (nxtObj instanceof SQLObject) {
							...
							ToggleEnforcedOnRelationshipsDBM(sqlObj);
						}
					}
				}
			...
			private void ToggleEnforcedOnRelationshipsDBM(SQLObject sqlObject) {
				// correct enforced flag on FK's if instance of table
				if (sqlObject instanceof Table) {
					toggleEnforced(sqlObject);
				}
				// get tables if instance of schema
					...
					while (validChildrenIter.hasNext()) {
						...
						if (nextObj instanceof SQLObject) {
							ToggleEnforcedOnRelationshipsDBM((SQLObject) nextObj);
						}
					}
					// correct enforced flag on FK's if instance of table
					toggleEnforced(sqlObject);
			...
			private void toggleEnforced(SQLObject sqlObject) {
				if (sqlObject instanceof BaseTable) {
					...
					while (tableIter.hasNext()) {
						...
						while (FKIter.hasNext()) {
							...
							if (fKey.isEnforced()) {
								// Sets the enforced flag on a foreign key to false if set to true
